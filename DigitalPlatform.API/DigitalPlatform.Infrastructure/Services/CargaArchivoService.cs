...
public class CargaArchivoService : ICargaArchivoService
{
    ...
    private int GuardarBaseClientes(List<Dictionary<string, object>> rows, int cargaArchivoId)
    {
        var entities = rows.Select(row => new BaseCliente
        {
            ...
            NumeroCliente = GetStr(row, ["B"]),
            Nombre = GetStr(row, ["C"]),
            CampoClasificacion = GetStr(row, ["D"]),
            ContactoContabilidad = GetStr(row, ["E"]),
            ContactoTesoreria = GetStr(row, ["F"]),
            ContactoFinanzas = GetStr(row, ["G"]),
            ContactoOperacion = GetStr(row, ["H"]),
            ContactoComercial = GetStr(row, ["I"]),
            ContactoCompras = GetStr(row, ["J"]),
            Telefono = GetStr(row, ["K"]),
            CorreoElectronico = GetStr(row, ["L"]),
            ModificadoPor = GetStr(row, ["M"]),
            FechaModificacion = GetDate(row, ["N"]),
            IdPais = GetStr(row, ["O"]),
            Pais = GetStr(row, ["P"]),
            IdTipoNif = GetStr(row, ["Q"]),
            TipoNif = GetStr(row, ["R"]),
            IdClaseImpuesto = GetStr(row, ["S"]),
            ClaseImpuesto = GetStr(row, ["T"]),
            PersonaFisica = GetStr(row, ["U"]),
            Poblacion = GetStr(row, ["V"]),
            IdGrupoClientes = GetStr(row, ["W"]),
            GpoClientes = GetStr(row, ["X"]),
            IdAddenda = GetStr(row, ["Y"]),
            Addenda = GetStr(row, ["Z"]),
        }).ToList();
